...
1. ~~**`GOVERNANCE.md:15-17` tells consumers the published conformance suite is `assert_frame_contract`, `assert_index_alignment_laws`, `assert_cross_level_alignment_law`.**~~ **DONE 2026-08-17 (S3 #243).** `conformance.__all__` exports six names — those three plus `CONFORMANCE_FLOOR`, **`assert_frame_envelope`** and **`assert_reindex_fill_law`**. This is the document that tells a consumer what to run in its own CI, which is ADR-016's entire mechanism. A consumer following it literally runs three of five checks, and the two it skips are the shipped mitigation for C-46 and the law pinning ADR-026.
   *Resolution of item 1:* `GOVERNANCE.md` now carries a three-row table naming every published name across all three conformance modules — including `assert_reconcile_contract`, which the old prose omitted at the *module* level as well as the function level — and states plainly that **each module's `__all__` is the source of truth, not the table**, with the command to read it. The "what the floor tracks" bullet now says "every name in the table above" instead of repeating a third partial list; its semantics are untouched, because that is where issue #237 lands. Verified in both directions: no published name missing from `GOVERNANCE.md`, and no name claimed by `GOVERNANCE.md` that is not published.
