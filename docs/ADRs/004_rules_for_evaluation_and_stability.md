...
Until then, change is governed by those ADRs and by careful review. The strongest signal
that it is time to activate this ADR is the publication of **`v0.1.0`** with at least one
consumer pinning the package: at that moment the SemVer policy sketched for an N-repo leaf
(README §8 — MAJOR = remove/rename a field, change a dtype/axis meaning, add a *required*
identifier; MINOR = a new frame, optional metadata key, or `io/` format) and the
conformance-floor/ownership governance (README §13b) should be ratified here.
