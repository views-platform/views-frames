...
- **ADR-002** — [Topology and Dependency Rules](002_topology_and_dependency_rules.md). The leaf at the root of the platform DAG (every arrow points toward it; imports nothing internal; two-leaves rule with `views-appwrite`) and the intra-package layering, lowest layer first (`_typing`/`metadata`/`spatial_level` → `_validation`/`io` → `index` → `protocols` → the frames, with `conformance` a top-layer consumer). **Amended 2026-08-17** (register C-82): `io/` is a codec the frames call, not a layer above them.
...
- **ADR-018** — [API freeze at v1.0.0](018_api_freeze_v1.md). v1.0.0 freezes the public surface (frames + index + protocols + conformance + summarizer estimators); ends the pre-1.0 breaking-in-MINOR latitude; records the frozen `(time, unit)` cross-level key, the row-uniqueness stance, and `select`/`reindex`. Resolves C-27 (with GOVERNANCE). **Amended 2026-08-18 (ADR-028):** the freeze was broken once, deliberately, in the 2.0.0 MAJOR — the constructors now reject a non-index `index`. No name was removed or re-signatured; the set of never-valid inputs narrowed.
